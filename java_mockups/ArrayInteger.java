...
public class ArrayInteger {
    ...
    public boolean isLargerThan(ArrayInteger num2){
        if(this.sign != num2.sign){ //one is positive and the other is negative 
            return this.sign==1; //returns true if the sign is positive and false otherwise (can only be positive or negative)
        }
        ...
        if(this.digits.size() != num2.digits.size()){
            //imbalance of sizes
            //returns true if positive sign and more letters or negative sign and less letters
            return ((sign==1 && this.digits.size() > num2.digits.size()) || (sign==-1 && this.digits.size() < num2.digits.size()));
        }
        ...
        for(int i = this.digits.size()-1; i >= 0 ; i--){
            ...
            if(this.digits.get(i) != num2.digits.get(i)){
                return ((sign==1 && this.digits.get(i) > num2.digits.get(i)) || (sign ==-1 && this.digits.get(i) < num2.digits.get(i))); //will return true if positive sign and number is greater, or negative sign and number is less
            }
        }
        //if has not returned true until here - they are equal and therefore it will return false
        return false;
    }
    ...
    public static boolean isAbsoluteArrayLarger(ArrayList<Integer> a1, ArrayList<Integer> a2){
        //if the sizes are different then it will return true if a1 is longer and false if s1 is not longer
        if(a1.size() != a2.size()){
    ...
    public static int[] divideInternal(ArrayInteger numerator, ArrayInteger denominator){
        ...
        // remainder.sign = remainder.sign * remainder.sign; //this will ensure the absolute value of the sign is taken so the remainder won't be negative
    ...
    public static ArrayInteger add(ArrayInteger curr, ArrayInteger adder){ //make this private
        //Takes a copy of the current arraylists and will reset curr_num and num2 with these after performing the addition
        //this is to prevent side effects and changing the value since it is passed by reference and not value 
        ...
        if(curr.sign != adder.sign){
            if ((curr.sign == -1 && isAbsoluteArrayLarger(curr.digits, adder.digits)) || (adder.sign == -1 && isAbsoluteArrayLarger(adder.digits, curr.digits))){
            ...
            curr.sign = 1; //this treats double negatives as a normal addition and works out the signs at the end
            adder.sign = 1;
        }
        ...
        int i = 0;
        while((curr.digits.size() > i && adder.digits.size() > i) || (curr.digits.size() > i && carry != 0) || (adder.digits.size() > i && carry != 0)){ //loops while both have column i
        ...
        //while loop for each arraylist - if there is left over information then it will just append it to the output list 
        ...
        //reset the values of the array integers to oringals saved at the start of the method
